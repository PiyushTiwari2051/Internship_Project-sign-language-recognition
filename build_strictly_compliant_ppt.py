...
def build_strictly_compliant_ppt():
    template_path = r'c:\Users\HP\Downloads\Internship Presentation PPTs Template Final.pptx'
    output_path = r'c:\Users\HP\Downloads\Piyush_Tiwari_Sign_Language_Recognition_True_Blue_Links.pptx'
    workspace_copy = r'Piyush_Tiwari_Sign_Language_Recognition_True_Blue_Links.pptx'

    prs = Presentation(template_path)

    # Color Palette
    NAVY = RGBColor(15, 23, 42)          # #0f172a
    DARK_ROYAL_BLUE = RGBColor(0, 51, 204) # #0033cc (Deep Dark Royal Blue for links)
    ...
    # ==================== CRITICAL: Fix PowerPoint Theme Hyperlink Colors ====================
    # Replace light purple (CCCCFF) in theme XML with Deep Dark Royal Blue (0033CC)
    for part in prs.part.package.iter_parts():
        if 'theme' in part.partname:
            blob = part.blob
            blob = blob.replace(b'val="CCCCFF"', b'val="0033CC"')
            blob = blob.replace(b'val="B2B2B2"', b'val="002060"')
            part._blob = blob

    # ==================== SLIDE 1: Precise In-Place Template Modification ====================
    slide1 = prs.slides[0]
    ...
    # Update Student Name & Roll No in Shape 1 without destroying runs/tabs
    shape1 = slide1.shapes[1]
    tf1 = shape1.text_frame
    
    if len(tf1.paragraphs) > 1:
        p1 = tf1.paragraphs[1]
        for r in p1.runs:
            if 'Name of the Student' in r.text:
                r.text = r.text.replace('Name of the Student', 'Piyush Tiwari')

    if len(tf1.paragraphs) > 2:
        p2 = tf1.paragraphs[2]
        if len(p2.runs) > 0:
            p2.runs[0].text = '   2100270100000'
        if len(p2.runs) > 1:
            p2.runs[1].text = '\t\t'

    # Delete existing template slides 2 to end while preserving Slide 1
    while len(prs.slides) > 1:
        rId = prs.slides._sldIdLst[1].rId
        prs.part.drop_rel(rId)
        del prs.slides._sldIdLst[1]

    # Helper function for Slide Titles (RULE 1: 32 pt Bold)
    def add_slide_header(slide, title_text, slide_num):
        ...
        p.text = title_text
        p.font.name = 'Calibri'
        p.font.size = Pt(32) # STRICT COMPLIANCE: 32 pt (Bold)
        p.font.bold = True
        p.font.color.rgb = NAVY

        # Slide Number Footer (Safe Position at bottom right)
        num_box = slide.shapes.add_textbox(Inches(8.5), Inches(6.5), Inches(0.8), Inches(0.3))
    ...
    # Helper function for Bullets (RULE 3: 20-24pt for Main Text, RULE 4: 18-20pt for Sub-points, NEVER below 18pt)
    def add_bullets(slide, items, left=0.6, top=1.3, width=8.8, height=4.8):
        ...
        for idx, item in enumerate(items):
            ...
            if isinstance(item, tuple):
                ...
                p.text = "• " + heading + ": "
                p.font.name = 'Calibri'
                p.font.size = Pt(20) # Main Lead-in: 20 pt (Bold)
                p.font.bold = True
                p.font.color.rgb = NAVY
                ...
                run.font.size = Pt(18) # Sub-point Body: 18 pt (Never below 18pt)
                run.font.bold = False
                run.font.color.rgb = DARK_GRAY
            else:
                p.text = "• " + item
                p.font.name = 'Calibri'
                p.font.size = Pt(20) # Bullet Text: 20 pt
                p.font.bold = False
                p.font.color.rgb = DARK_GRAY

    # ==================== SLIDE 2: 2-Column Outline ====================
    ...
    col1_items = [
        ...
        ("5. Keypoint Extraction", "Spatial landmark extraction (Hands & Pose)")
    ]

    col2_items = [
        ("6. Dataset & Feature Details", "Sliding window aggregation (18 statistical features)"),
        ("7. Sequence LSTM Classifier", "Multi-layer LSTM model architecture & loss optimization"),
        ("8. Desktop GUI & Speech Translation", "Native Python Tkinter app & Pyttsx3 speech synthesis"),
        ("9. Results & Performance Analysis", "Accuracy (98.67%), F1-Score (0.99), & per-class table"),
        ("10. Conclusion & References", "Summary of achievements, future scope & academic references")
    ]

    add_bullets(slide2, col1_items, left=0.6, top=1.3, width=4.2, height=4.8)
    add_bullets(slide2, col2_items, left=5.0, top=1.3, width=4.4, height=4.8)
    ...
    cap6 = slide6.shapes.add_textbox(Inches(0.6), Inches(5.9), Inches(8.8), Inches(0.4))
    ...
    # ==================== SLIDE 8: Dataset & Feature Details ====================
    ...
    rows, cols = 13, 5
    left, top, width, height = Inches(0.6), Inches(1.3), Inches(8.8), Inches(4.5)
    ...
    cap13 = slide13.shapes.add_textbox(Inches(0.6), Inches(6.0), Inches(8.8), Inches(0.4))
    ...
    # ==================== SLIDE 17: References & Deep Royal Blue Hyperlinks ====================
    slide17 = prs.slides.add_slide(prs.slide_layouts[6])
    add_slide_header(slide17, "References & Academic Literature", 17)

    references_data = [
        ("1. MediaPipe Perception Pipeline Framework", 
         "Lugaresi et al., arXiv:1906.08172, 2019.", 
         "https://arxiv.org/abs/1906.08172"),
        
        ("2. Long Short-Term Memory (LSTM) Architecture", 
         "Hochreiter & Schmidhuber, Neural Computation 9(8), 1997.", 
         "https://doi.org/10.1162/neco.1997.9.8.1735"),
        
        ("3. Word-Level Deep Sign Language Recognition", 
         "Li et al., IEEE/CVF CVPR, 2020.", 
         "https://openaccess.thecvf.com/content_CVPR_2020/html/Li_Word-Level_Deep_Sign_Language_Recognition_CVPR_2020_paper.html"),
        
        ("4. Google MediaPipe Vision Landmark Documentation", 
         "Google AI Edge Solutions, Hand & Pose API, 2026.", 
         "https://ai.google.dev/edge/mediapipe/solutions/vision/hand_landmarker"),
        
        ("5. Keras 3 & TensorFlow Recurrent Layers API", 
         "TensorFlow / Keras Documentation, 2026.", 
         "https://keras.io/api/layers/recurrent_layers/lstm/"),
        
        ("6. OpenCV Real-Time Computer Vision Library", 
         "OpenCV Open Source Foundation Documentation, 2026.", 
         "https://docs.opencv.org/")
    ]

    tb17 = slide17.shapes.add_textbox(Inches(0.6), Inches(1.3), Inches(8.8), Inches(5.0))
    tf17 = tb17.text_frame
    tf17.word_wrap = True

    for idx, (title, citation, url) in enumerate(references_data):
        # Paragraph 1: Bullet Title & Citation
        p = tf17.paragraphs[0] if idx == 0 else tf17.add_paragraph()
        p.space_after = Pt(2)
        ...
        r_cite = p.add_run()
        r_cite.text = citation
        r_cite.font.name = 'Calibri'
        r_cite.font.size = Pt(17)
        r_cite.font.bold = False
        r_cite.font.color.rgb = DARK_GRAY

        # Paragraph 2: Full URL in Deep Royal Dark Blue (#0033cc)
        p_url = tf17.add_paragraph()
        ...
        r_link = p_url.add_run()
        r_link.text = url
        r_link.font.name = 'Calibri'
        r_link.font.size = Pt(16)
        r_link.font.bold = True
        r_link.font.underline = True
        r_link.font.color.rgb = DARK_ROYAL_BLUE # High-Contrast Deep Dark Blue (#0033cc)
    ...
    p18_2.font.color.rgb = DARK_ROYAL_BLUE
    ...
    print(f"TRUE BLUE LINK PRESENTATION saved successfully to:\n1. {output_path}\n2. {workspace_copy}")

if __name__ == '__main__':
    build_strictly_compliant_ppt()
